...
VERSION = "0.3"
OUT_VERIFY = "STOCRS_R_Demo_v0_3_VERIFY.txt"
OUT_JSON = "STOCRS_R_Demo_v0_3.json"
...
def add_enhancement_coupon(program, coupon_amount):
    upgraded = deepcopy(program)
    upgraded["LOYALTY_COUPON"] = {"deps": ["AFTER_DISCOUNT"], "func": lambda v: coupon_amount if v["AFTER_DISCOUNT"] >= 250 else 0}
...
def run_demo(seed):
    ...
    enhanced_program_v1 = add_enhancement_coupon(base_program, coupon_amount=15)
    enhanced_nodes_v1 = list(enhanced_program_v1.keys())
    enhanced_run_v1 = resolve(enhanced_program_v1, shuffled(enhanced_nodes_v1, seed + 2026))

    enhanced_program_v2 = add_enhancement_coupon(base_program, coupon_amount=25)
    enhanced_nodes_v2 = list(enhanced_program_v2.keys())
    enhanced_run_v2 = resolve(enhanced_program_v2, shuffled(enhanced_nodes_v2, seed + 2027))

    base_total = run_a["values"].get("FINAL_TOTAL")
    enhanced_total_v1 = enhanced_run_v1["values"].get("FINAL_TOTAL")
    enhanced_total_v2 = enhanced_run_v2["values"].get("FINAL_TOTAL")
    ...
    output = {
        ...
        "functional_update_story": "Only the coupon policy value changed from 15 to 25. The resolver and module structure remained intact.",
        "functional_update_changed_policy_lines": 1,
        ...
        "enhanced_final_total_v1": enhanced_total_v1,
        "enhanced_final_total_v2": enhanced_total_v2,
        "enhancement_v1_improves_total": enhanced_total_v1 < base_total,
        "enhancement_v2_improves_total": enhanced_total_v2 < enhanced_total_v1,
        "enhancement_v1_savings": round(base_total - enhanced_total_v1, 2),
        "enhancement_v2_additional_savings": round(enhanced_total_v1 - enhanced_total_v2, 2),
        "enhancement_v2_total_savings_from_base": round(base_total - enhanced_total_v2, 2),
    }
...
def write_outputs(result):
    ...
    with open(OUT_VERIFY, "w", encoding="utf-8") as f:
        f.write("STOCRS-R Demo v0.3 VERIFY\n")
        ...
        f.write("Functional update story: " + result["functional_update_story"] + "\n")
        f.write("Functional update changed policy lines: " + str(result["functional_update_changed_policy_lines"]) + "\n\n")
        ...
        f.write("Enhanced final total v1: " + str(result["enhanced_final_total_v1"]) + "\n")
        f.write("Enhanced final total v2: " + str(result["enhanced_final_total_v2"]) + "\n")
        f.write("Enhancement v1 improves total: " + str(result["enhancement_v1_improves_total"]) + "\n")
        f.write("Enhancement v2 improves total: " + str(result["enhancement_v2_improves_total"]) + "\n")
        f.write("Enhancement v1 savings: " + str(result["enhancement_v1_savings"]) + "\n")
        f.write("Enhancement v2 additional savings: " + str(result["enhancement_v2_additional_savings"]) + "\n")
        f.write("Enhancement v2 total savings from base: " + str(result["enhancement_v2_total_savings_from_base"]) + "\n\n")
        ...
        f.write("Enhanced v1 certificate: " + result["enhanced_coupon_run_v1"]["certificate"] + "\n")
        f.write("Enhanced v2 certificate: " + result["enhanced_coupon_run_v2"]["certificate"] + "\n")
...
def main():
    ...
    print()
    print("Functional update story:")
    print(result["functional_update_story"])
    print("Changed policy lines:", result["functional_update_changed_policy_lines"])
    print()
    ...
    print("Enhancement v1 improves total:", "YES" if result["enhancement_v1_improves_total"] else "NO")
    print("Enhancement v2 improves total:", "YES" if result["enhancement_v2_improves_total"] else "NO")
    print("Enhancement v2 total savings from base:", result["enhancement_v2_total_savings_from_base"])
    print()
